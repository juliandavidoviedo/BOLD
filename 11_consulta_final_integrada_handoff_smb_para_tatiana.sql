-- Consulta final candidata Handoff Comercial SMB -> Customer Success/KAM
-- Version: 4.0 - integra base, TPV, terminales y jerarquia comercial
-- Motor esperado: Athena / Trino desde Metabase
--
-- IMPORTANTE DE SEGURIDAD
--   No seleccionar dim_crm_users.metadata. Esa columna puede contener secretos
--   tecnicos, incluyendo tokens. Esta consulta usa solo columnas explicitas.
--
-- Grano esperado:
--   1 fila por merchant_id.
--
-- Rutas de jerarquia usadas:
--   1. Merchant -> sales_agent_email -> dim_crm_users.parent_id -> Team Lead
--   2. Team Lead -> dim_crm_users.parent_id -> Manager
--   3. Merchant -> dim_client_georeference.hub_custom_id
--              -> dim_crm_pos_locations.custom_id -> hub_manager_email
--
-- Nota:
--   Bamboo no trae el nombre propio del usuario en las columnas descubiertas.
--   Para un ejecutivo, bamboo.reports_to suele representar el nombre del jefe
--   directo. Por eso se expone como team_lead_name_from_bamboo / manager_name_from_bamboo
--   y debe validarse contra la jerarquia comercial antes de productivizar.
...
-- Validacion recomendada despues de ejecutar:
-- 1. Confirmar que el resultado mantiene 10 filas para estos 10 merchants.
-- 2. Confirmar que no hay merchant_id duplicado.
-- 3. Revisar estado_jerarquia_comercial y estado_jerarquia_hub.
-- 4. Si la jerarquia comercial resuelve bien, reemplazar validation_merchants
--    por la tabla/base mensual de merchants del handoff.
