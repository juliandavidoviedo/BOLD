...
base_integrada AS (
    SELECT
        ...
        CASE
            WHEN coalesce(tf.tpv_m1, 0) = 0 THEN NULL
            ELSE (coalesce(tf.tpv_m0, 0) - coalesce(tf.tpv_m1, 0)) / coalesce(tf.tpv_m1, 0)
        ...
        tf.ultimo_mes_con_tpv,
        coalesce(tf.meses_con_tpv, 0) AS meses_con_tpv,
...
SELECT
    ...
    tpv_otros_bancos_m0,
    tpv_otros_bancos_m1,
    crecimiento_m0_vs_m1,
    crecimiento_ultimos_3m_vs_previos_3m,
    ultimo_mes_con_tpv,
    meses_con_tpv,
    tpv_total_m0_growth,
    tpv_total_m1_growth,
