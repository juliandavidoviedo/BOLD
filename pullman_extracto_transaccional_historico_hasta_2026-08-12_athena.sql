...
movement_base AS (
    SELECT
        ...
        ROW_NUMBER() OVER (
            ...
            ORDER BY
                m."load_datetime" DESC,
                m."internal_event_id" DESC
        ) AS rn
...
SELECT
    ...
    CASE
        ...
        WHEN p."status" = 'CHARGEBACK' THEN 'CONTRACARGO'
        WHEN p."status" = 'DISPUTED' THEN 'DISPUTADO'
        WHEN p."status" = 'REJECTED' THEN 'RECHAZADO'
        WHEN p."status" = 'VOIDED' THEN 'ANULADO'
        WHEN p."status" IS NULL THEN 'NO DISPONIBLE'
        ELSE p."status"
    END AS "OPERACIÓN",
    ...
    COALESCE(
        CAST(m."withholding_at_source" AS DECIMAL(38,2)),
        CAST(0 AS DECIMAL(38,2))
    ) AS "VALOR RETE FUENTE",

    COALESCE(
        CAST(m."vat_withholding" AS DECIMAL(38,2)),
        CAST(0 AS DECIMAL(38,2))
    ) AS "VALOR RETE IVA",

    COALESCE(
        CAST(m."ica_withholding" AS DECIMAL(38,2)),
        CAST(0 AS DECIMAL(38,2))
    ) AS "VALOR RETE ICA",
